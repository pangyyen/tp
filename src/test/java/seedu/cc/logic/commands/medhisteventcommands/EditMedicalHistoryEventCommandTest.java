...
public class EditMedicalHistoryEventCommandTest {
    ...
    public void execute_allFieldsSpecifiedUnfilteredList_success() {
        ...
        EditMedicalHistoryEventDescriptor descriptor = new EditMedicalHistoryEventDescriptorBuilder(editedMedicalHistoryEvent).build();
        EditMedicalHistoryEventCommand editCommand = new EditMedicalHistoryEventCommand(INDEX_FIRST_MEDHISTEVENT, INDEX_FIRST_PATIENT, descriptor);

        Patient patient = model.getFilteredPatientList().get(INDEX_FIRST_PATIENT.getZeroBased());
        model.listMedicalHistoryEvents(patient);

        String expectedMessage = String.format(EditMedicalHistoryEventCommand.MESSAGE_EDIT_EVENT_SUCCESS, editedMedicalHistoryEvent);
        ...
        expectedModel.setMedicalHistoryEvent(patient, model.getFilteredMedicalHistoryEventList().get(0), editedMedicalHistoryEvent);
    ...
    public void execute_someFieldsSpecifiedUnfilteredList_success() {
        ...
        Patient editedPatient = patientInList.withMedicalHistory(new MedicalHistoryEventBuilder().buildMedicalHistory()).build();

        Index indexLastMedicalHistoryEvent = Index.fromOneBased(model.getFilteredMedicalHistoryEventList().size());
        MedicalHistoryEvent lastMedicalHistoryEvent = model.getFilteredMedicalHistoryEventList().get(indexLastMedicalHistoryEvent.getZeroBased());

        EditMedicalHistoryEventDescriptor descriptor = new EditMedicalHistoryEventDescriptorBuilder().withMedicalCondition(VALID_MEDICAL_CONDITION)
                .withDate(VALID_DATE).build();
        EditMedicalHistoryEventCommand editCommand = new EditMedicalHistoryEventCommand(indexLastMedicalHistoryEvent, INDEX_FIRST_PATIENT, descriptor);
    ...
    public void execute_invalidMedicalHistoryEventIndexUnfilteredList_failure() {
        ...
        EditMedicalHistoryEventDescriptor descriptor = new EditMedicalHistoryEventDescriptorBuilder().withMedicalCondition(VALID_MEDICAL_CONDITION).build();
        EditMedicalHistoryEventCommand editCommand = new EditMedicalHistoryEventCommand(outOfBoundIndex, INDEX_FIRST_PATIENT, descriptor);
